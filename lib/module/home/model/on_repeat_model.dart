...
class OnRepeatModel {
  OnRepeatModel({required this.song, this.playCount = 0});

  final ListenMusicDetailsModel song;

  /// The user's own play count for this song, not the song's global one.
  final int playCount;

  factory OnRepeatModel.fromJson(Map<String, dynamic> json) {
    final song = json['song'] is Map
        ? Map<String, dynamic>.from(json['song'] as Map)
        : <String, dynamic>{};

    return OnRepeatModel(
      song: ListenMusicDetailsModel.fromJson(song),
      playCount: OnRepeatPage._asInt(json['playCount']),
    );
  }
...
/// One page of the on-repeat list, with the counters the list needs to ask
/// for the next.
class OnRepeatPage {
  const OnRepeatPage({
    this.items = const [],
  ...
  final List<OnRepeatModel> items;
  ...
  factory OnRepeatPage.fromJson(Map<String, dynamic> json) {
    ...
    final items = entries
        .whereType<Map>()
        .map((item) => OnRepeatModel.fromJson(Map<String, dynamic>.from(item)))
        .toList();
