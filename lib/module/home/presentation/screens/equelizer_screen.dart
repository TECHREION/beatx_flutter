...
class _EqualizerScreenState extends State<EqualizerScreen> {
  ...
  List<double> _gains = List<double>.from(
    _kPresetGains['Small Speakers']!,
  );

  void _selectPreset(int index) {
    final name = _kPresets[index];
    setState(() {
      _selectedPreset = index;
      _gains = List<double>.from(_kPresetGains[name]!);
    });
  }
      ...
      body: SafeArea(
        child: Column(
          children: [
            ...
            // EQ curve — fixed height
            _EqSection(
              gains: _gains,
              onGainChanged: _updateGain,
            ),
    ...
    return SizedBox(
      ...
      child: LayoutBuilder(
        builder: (context, constraints) {
          ...
          // X positions for 6 bands, evenly distributed
          ...
          return Stack(
            children: [
              ...
              // Curve painter
              CustomPaint(
                size: Size(w, visHeight),
                painter: _EqPainter(gains: gains, xs: xs, height: visHeight),
              ),
              // Draggable handles
              for (int i = 0; i < 6; i++)
                Positioned(
                  left: xs[i] - 13,
                  top: gains[i] * visHeight - 13,
                  child: GestureDetector(
                    onPanUpdate: (d) =>
                        onGainChanged(i, d.delta.dy, visHeight),
...
class _EqGlow extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
...
class _EqPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final centerPaint = Paint()
      ...
      ..strokeWidth = 1;
    canvas.drawLine(Offset(0, height / 2), Offset(size.width, height / 2), centerPaint);

    // Build points clamped so handles stay in view
    ...
    final fillPath = _buildCurvePath(pts);
    fillPath.lineTo(pts.last.dx, height);
    fillPath.lineTo(pts.first.dx, height);
    fillPath.close();
    ...
    final curvePath = _buildCurvePath(pts);
...
class _FreqLabels extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(
        children: [
          for (int i = 0; i < _kFreqLabels.length; i++) ...[
            Expanded(
              child: Text(
                _kFreqLabels[i],
                textAlign: TextAlign.center,
...
class _ProgressBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final w = constraints.maxWidth;
        final scrubberX = (w * progress).clamp(6.0, w - 6.0);
