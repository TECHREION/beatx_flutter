...
/// The user's listening history, over `GET /songs/recently-played`.
///
/// Newest first, as the backend orders it. Pages are pulled in as the list is
/// scrolled, so the screen ends up holding the whole history.
class RecentlyPlayedScreen extends StatelessWidget {
  const RecentlyPlayedScreen({super.key});

  static const accent = Color(0xFFC88BFF);

  static const _artGradient = [Color(0xFF7B3BFF), Color(0xFF40DDEB)];
  ...
  Widget build(BuildContext context) {
    final ctrl = RecentlyPlayedController.instance;
        ...
        child: Column(
          children: [
            const LikedHeader(
              ...
              subtitle: 'Everything you have listened to, newest first',
            ...
            Obx(
              () => LikedSummaryStrip(
                ...
                // song, this picks up where the history left off.
                        ...
                        if (ctrl.entries.isEmpty) {
                          return SliverFillRemaining(
                            ...
                            child: LikedEmptyState(
                              ...
                              failedTitle: 'Could not load your history',
...
class _HistoryRow extends StatelessWidget {
  ...
  final RecentlyPlayedModel entry;
  final int position;
  final RecentlyPlayedController controller;
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      return LikedRowCard(
        position: position,
        playing: playing,
        accent: RecentlyPlayedScreen.accent,
        coverUrl: song.coverUrl,
        fallbackIcon: Icons.music_note_rounded,
        gradient: RecentlyPlayedScreen._artGradient,
        title: song.title,
  ...
  /// The pill under the captions, dropped when the entry carried no date.
  String _playedAt() {
    final when = RecentlyPlayedController.formatLastPlayed(entry.lastPlayedAt);
    return when.isEmpty ? '' : 'Played $when';
  }
}
